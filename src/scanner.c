...
void *tree_sitter_sql_orcl_external_scanner_create() {
  LexerState *state = malloc(sizeof(LexerState));
  state->start_tag = NULL;
  return state;
}

void tree_sitter_sql_orcl_external_scanner_destroy(void *payload) {
...
unsigned tree_sitter_sql_orcl_external_scanner_serialize(void *payload, char *buffer) {
  LexerState *state = (LexerState *)payload;
...
void tree_sitter_sql_orcl_external_scanner_deserialize(void *payload, const char *buffer, unsigned length) {
  LexerState *state = (LexerState *)payload;
  state->start_tag = NULL;
